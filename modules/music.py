...
class Music(commands.Cog):
    ...
    stage_flags.add_argument('-position', '-pos', '-p', type=int, default=0, help='Colocar a música em uma posição específica.\nEx: -p 10')
    ...
    stage_flags.add_argument('-source', '-scr', type=str, default="ytsearch", help='Fazer a busca da música usando uma fonte específica [youtube/soundcloud etc]')
    ...
    async def player_controller(self, interaction: disnake.MessageInteraction, control: str, **kwargs):
        ...
        try:
            ...
            if control == PlayerControls.integration_manager:
                ...
                cmd = self.bot.pool.controller_bot.get_slash_command("integration").children.get("manager")
                await self.process_player_interaction(interaction, cmd, cmd_kwargs)
                return
