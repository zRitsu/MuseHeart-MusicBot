...
class ErrorHandler(commands.Cog):
    ...
    async def process_interaction_error(self, inter: disnake.AppCmdInter, error: Exception):
        ...
        if not error_msg:
            ...
            kwargs["embed"] = disnake.Embed(
                color=color,
                title = "Komutta bir hata oluştu:",
                description=f"```py\n{repr(error)[:2030].replace(self.bot.http.token, 'mytoken')}```"
            )

            if self.bot.config["AUTO_ERROR_REPORT_WEBHOOK"]:
                send_webhook = True
                kwargs["embed"].description += " `Geliştiricim sorunla ilgili bilgilendirilecek.`"
    ...
    async def on_legacy_command_error(self, ctx: CustomContext, error: Exception):
        ...
        if isinstance(error, commands.NotOwner):
            print(f"{ctx.author} [{ctx.author.id}] komutu kullanacak botun sahibi değil: {ctx.command.name}")
            return
    ...
    async def on_error_report(self, inter: disnake.MessageInteraction):
        ...
        if str(inter.author.id) not in inter.message.content:
            await inter.send(f"Yalnızca kullanıcı {inter.message.content} bu düğmeyi kullanabilirsiniz!", ephemeral=True)
            return
        ...
        await inter.response.send_modal(
            ...
            components=[
                ...
                disnake.ui.TextInput(
                    ...
                    label="Hata resmi/yazdırma bağlantısı (İsteğe bağlı)",
                    custom_id="image_url",
                    max_length=300,
                    required=False
                )
    ...
    async def on_report_submit(self, inter: disnake.ModalInteraction):
        ...
        if not inter.message.embeds:
            await inter.response.edit_message(
                embed=disnake.Embed(
                    title="Mesaj yerleştirme kaldırıldı!",
        ...
        if image_url and not URL_REG.match(image_url):
            await inter.send(
                embed=disnake.Embed(
                    title="Geçersiz resim bağlantısı!",
                    description=inter.text_values["error_details"]
                ), ephemeral=True
            )
            return
        ...
        await inter.response.edit_message(
            embed=disnake.Embed(
                description="**Hata başarıyla bildirildi!**",
                color=self.bot.get_color(inter.guild.me)
            ), view=None
        )
        ...
        embed.set_author(name=f"Hata bildirildi: {inter.author} - {inter.author.id}", icon_url=user_avatar)

        guild_txt = f"Sunucu: {inter.guild.name} [{inter.guild.id}]"
    ...
    def build_report_embed(self, ctx):
        ...
        if ctx.guild:
            ...
            if vc := ctx.author.voice:
                embed.add_field(
                    name="Ses kanalı (kullanıcı):", inline=False,
        ...
        try:
            ...
            embed.description = f"**Eğik Çizgi Komutu:**```\n{ctx.data.name}``` "

            if ctx.filled_options:
                embed.description += "**Seçenekler**```\n" + \
