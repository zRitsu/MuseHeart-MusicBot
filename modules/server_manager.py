...
class ServerManagerView(disnake.ui.View):
    ...
    def build_embed(self, interaction: Union[disnake.MessageInteraction, CustomContext]):
        ...
        embed = disnake.Embed(
            ...
            description=f"```{self.current_guild.name}```\n"
                        ...
                        f"**İçinde yaratıldı:** <t:{created_at}:f> - <t:{created_at}:R>\n"
                        f"**O zamandan beri üyeyim:** <t:{joined_at}:f> - <t:{joined_at}:R>\n"
        ...
        if len(self.pages) > 1:
            embed.title = f"Geçerli sayfa: [{self.current_page+1}/{len(self.pages)}]"
    ...
    def rebuild_components(self):
        ...
        if has_server_select and len(self.pages) > 1:
            ...
            back = disnake.ui.Button(label="Geri", emoji="⬅️")
            back.callback = self.previous_page
            self.add_item(back)

            next = disnake.ui.Button(label="Sonraki", emoji="➡️")
        ...
        stop = disnake.ui.Button(label="Dur", emoji="⏹️", style=disnake.ButtonStyle.blurple)
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction) -> bool:
        ...
        if interaction.author.id != self.inter.user.id:
            await interaction.response.send_message("Burada etkileşime geçemezsin...", ephemeral=True)
            return False
    ...
    async def leave_guild(self, interaction: disnake.MessageInteraction):
        ...
        if interaction.guild.id == self.current_guild.id and interaction.bot.user.id == self.bot.user.id:
            await interaction.response.edit_message(
                embed=disnake.Embed(
                    description="**Beni mevcut sunucudan çıkardınız.**",
...
class ServerManagerCog(commands.Cog):
    ...
    @commands.command(name="servers", aliases=["servermanager"], hidden=True,
                      description="Botun üzerinde bulunduğu sunucuları yönetin.")
